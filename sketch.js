...
function preload(){
  ...
  youWonImg = loadImage("images/you won.png")
}
...
function setup() {
  ...
  heart2.scale = 0.5;
  heartArray = [heart,heart1,heart2];
...
function draw() {
    ...
    if (gameState==="PLAY"){
     ...
     // console.log("monster.x",monster.x);
      //console.log("hi",monster.x+monster.width/2)
          ...
          // Destroy heart on player touching the monster
          if(player.isTouching(monsterArray)){
            player.x = 50;
            player.y = 400;
            heartArray[heartIndex].destroy();
          ...
          if(score===18){
            image(youWonImg,displayWidth/2,displayHeight/2);

          }

          //Destroy fruit on playing touching the fruit
  ...
  image(scoreImg,displayWidth/2-100,10);
  textSize(20);
  fill("green")
  text(score,displayWidth/2,35);
